...
const statusCfg = {
  ...
  Confirmed: { bg:"rgba(16,185,129,0.10)",  text:"#065F46", dot:"#10B981", border:"rgba(16,185,129,0.25)" },
  Cancelled: { bg:"rgba(239,68,68,0.10)",   text:"#B91C1C", dot:"#EF4444", border:"rgba(239,68,68,0.25)"  },
};
  ...
  return (
    <div className="fade-in">
      ...
      <div className="fade-in-up" style={{ marginBottom: 24 }}>
        <div className="page-eyebrow">Schedule Management</div>
        <h1 className="page-title">Appointments</h1>
        <p className="page-sub">
          {isAdmin ? "Manage and confirm all appointment requests." : "View all scheduled appointments."}
        </p>
      </div>
      ...
      <div className="fade-in-up stagger" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:22 }}>
        {[
          ...
          { label:"Confirmed", val:counts.Confirmed, gradient:"linear-gradient(135deg,#059669,#10B981)", shadow:"rgba(5,150,105,0.25)"  },
          { label:"Cancelled", val:counts.Cancelled, gradient:"linear-gradient(135deg,#DC2626,#EF4444)", shadow:"rgba(220,38,38,0.25)"  },
      ...
      {loading && (
        <div style={{ textAlign:"center", padding:"52px 24px" }}>
          ...
          <p style={{ color:"#8A92A6", fontSize:13 }}>Loading appointments…</p>
        </div>
      )}
      ...
      {!loading && filtered.length === 0 && (
        <div className="card" style={{ textAlign:"center", padding:"52px 24px" }}>
          ...
          <div style={{ fontWeight:600, color:"#1A2545", fontSize:15, marginBottom:6 }}>No appointments found</div>
          <p style={{ color:"#8A92A6", fontSize:13 }}>{search ? `No results for "${search}"` : `No appointments with status "${filterStatus}"`}</p>
        </div>
      )}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {filtered.map((a, idx) => {
          ...
          return (
            <div key={a.id} className="card fade-in-up" style={{ overflow:"hidden", padding:0, animationDelay:`${0.04+idx*0.03}s` }}>
              ...
              <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                ...
                <div style={{ flex:1, minWidth:180 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:"#1A2545", marginBottom:3 }}>{a.full_name}</div>
                ...
                {/* Status */}
                {isAdmin ? (
                  <select value={a.status} disabled={statusBusy[a.id]} onChange={e => changeStatus(a.id, e.target.value)} style={{ padding:"6px 10px", borderRadius:9, border:"1.5px solid rgba(200,148,42,0.20)", fontSize:12, fontWeight:600, cursor:"pointer", outline:"none", fontFamily:"'Poppins',sans-serif", flexShrink:0, background:"#fff", color:sc.text }}>
                ...
                {/* Expand */}
                <button onClick={() => setExpanded(open ? null : a.id)} style={{
              ...
              {open && (
                <div style={{ padding:"0 20px 20px", borderTop:"1px solid rgba(200,148,42,0.12)", animation:"fadeIn 0.2s ease" }}>
                  <div style={{ paddingTop:16, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:12 }}>
                    {[
                      { label:"Full Name",  val:a.full_name },
                      { label:"Mobile",     val:a.mobile },
                      { label:"Area",       val:a.area },
                      { label:"Purpose",    val:a.purpose },
                      { label:"Date",       val:d.toLocaleDateString("en-IN",{ weekday:"long", day:"numeric", month:"long", year:"numeric" }) },
                      { label:"Time Slot",  val:a.appointment_time },
                    ].map(({ label, val }) => (
